...
extern void rad_transfer_csm(double, double, double, double, double, double, double, double, const char*, const char*, const char*, const char*, const char*, int);
...
static PyObject* lightcurve_shock(PyObject* self, PyObject* args, PyObject* kw)
{
	...
	static char* argnames[] = {"E_ej", "M_ej", "M_ni", "n", "delta", "time_interval", "file_csm", "file_output", "D", NULL};
	if (!PyArg_ParseTupleAndKeywords(args, kw, "dddddd|ssi", argnames, &E_ej, &M_ej, &M_ni, &n, &delta, &time_interval, &file_csm, &file_output, &D))
		return NULL;
	shock_csm(E_ej, M_ej, M_ni, n, delta, time_interval, file_csm, file_output, D);
...
static PyObject* lightcurve_transfer(PyObject* self, PyObject* args, PyObject* kw)
{
	...
	static char* argnames[] = {"Eej", "Mej", "Mni", "n", "delta", "r_out", "time_interval", "plateau_flag", "file_csm", "file_shock", "file_lc", "file_lc_band", "dir_Lnu", "D", NULL};
	if (!PyArg_ParseTupleAndKeywords(args, kw, "dddddddd|sssssi", argnames, &Eej, &Mej, &Mni, &n, &delta, &r_out, &time_interval, &plateau_flag, &file_csm, &file_shock, &file_lc, &file_lc_band, &dir_Lnu, &D))
		return NULL;
	rad_transfer_csm(Eej, Mej, Mni, n, delta, r_out, time_interval, plateau_flag, file_csm, file_shock, file_lc, file_lc_band, dir_Lnu, D);
	return Py_BuildValue("");
}
