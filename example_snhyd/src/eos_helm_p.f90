...
!全ての値は引数渡し&返し
      subroutine eoshelm_p(n,dedlt_ar,temp_ar,e_ar,tau_ar,p_ar,x_ar,grv_ar,rad_ar,eu_ar,g_ar,g1_ar,cs_ar,u_ar,mn,nelem)
      include 'implno.dek'
...
! set the input vector. pipeline is only 1 element long in this example
       ...
       e_th = e_ar(k) -0.5d0*u_ar(k)**2 -grv_ar(k)*rad_ar(k)
...
! calculation i = 1
       ...
       if(done.eq.0)then
        call read_helm_table_p
        done = 1 !doneが1になっていれば次回以降read_helm_table_eをcallせずに済む
       end if

       error = 0
       call helmeos_p(k,error)
       if(error.eq.0)then
         eu_ar(k) = etot_row(1)
         e_ar(k) = etot_row(1) + 0.5d0*u_ar(k)**2 + grv_ar(k)*rad_ar(k)
!       write(*,*)"p_ar(k)",p_ar(k),k
         p_ar(k) = ptot_row(1)
!         dedlt_ar(k) = temp_ar(k)*cv_row(1)
       end if
       if(error.ne.0)then
         write(*,*)"eos error at j =",k
...
! routine read_helm_table_e reads an electron helm free energy table
! routine helmeos_e computes the pressure, energy and entropy via tables


      subroutine read_helm_table_p
      include 'implno.dek'
      include 'helm_table_storage.dek'

! this routine reads the helmholtz eos file, and
! must be called once before the helmeos_e routine is invoked.
...
! open the file (use softlinks to input the desired table)
       ...
       open(unit=19,file='snhyd/helm_table.dat',status='old')
...
!      write(6,*)
      ...
      subroutine read_helm_iontable_p
      include 'implno.dek'
      include 'helm_table_storage.dek'
...
! end of pipeline loop
      ...
      subroutine pretty_eos_out_p(whose)
      include 'implno.dek'
      include 'vector_eos.dek'
